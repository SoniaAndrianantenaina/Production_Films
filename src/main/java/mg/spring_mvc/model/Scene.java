...
//        return list;
//    }
    public String getDuree(HibernateDao dao) {
        return dao.getDuree(id);
    }

    public List<Scene> selectFinished(HibernateDao dao, int idfilm) throws Exception {
        List<Scene> list = new ArrayList<>();
        List<Object> obj = dao.selectFinishedScene(Scene.class,idfilm);
        for (int i = 0; i < obj.size(); i++) {
            list.add((Scene) obj.get(i));
        }
        return list;
    ...
    public ArrayList<Personnage> listPersonage(HibernateDao dao) {
        List<Personnage> obj = dao.getPerso(Personnage.class, this.id);
        ArrayList<Personnage> personnages = new ArrayList<>();
        for (int i = 0; i < obj.size(); i++) {
            personnages.add(obj.get(i));
        }
